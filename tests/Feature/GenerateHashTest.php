...
describe('execute() with custom slug', function () {
    test('accepts valid custom slug', function () {
        $slug = 'my-custom-slug';

        $hash = $this->action->execute($slug);

        expect($hash)->toBe($slug);
    });

    test('accepts alphanumeric slug', function () {
        $slug = 'abc123XYZ';
    ...
    test('accepts slug with dashes', function () {
    ...
    test('rejects slug shorter than 3 characters', function () {
        $this->action->execute('ab');
    })->throws(\InvalidArgumentException::class, 'Custom slug must be at least 3 characters');

    test('rejects slug longer than 50 characters', function () {
        $slug = str_repeat('a', 51);

        $this->action->execute($slug);
    })->throws(\InvalidArgumentException::class, 'Custom slug cannot exceed 50 characters');

    test('rejects slug with special characters', function () {
        $this->action->execute('my_slug!');
    })->throws(\InvalidArgumentException::class, 'Custom slug can only contain letters, numbers, and dashes');

    test('rejects slug with spaces', function () {
        $this->action->execute('my slug');
    })->throws(\InvalidArgumentException::class, 'Custom slug can only contain letters, numbers, and dashes');

    test('rejects slug from excluded words list', function () {
        // Note: Need to recreate action after config change for it to take effect
        config(['anon.excluded_words' => ['badword', 'offensive']]);
        $action = app(\App\Actions\Links\GenerateHash::class);

        $action->execute('badword');
    })->throws(\InvalidArgumentException::class, 'This custom slug is not allowed');

    test('rejects slug that already exists in database', function () {
        Link::factory()->create(['slug' => 'existing-slug']);

        $this->action->execute('existing-slug');
    })->throws(\InvalidArgumentException::class, 'This custom slug is already taken');

    test('slug validation is case-insensitive for excluded words', function () {
        // Note: Need to recreate action after config change for it to take effect
        config(['anon.excluded_words' => ['badword']]);
        $action = app(\App\Actions\Links\GenerateHash::class);
